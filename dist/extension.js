...
function estimateCost(tokens, costPerMillion) {
  return tokens / 1e6 * costPerMillion;
}
function rawDiffText(hunks) {
  return hunks.flatMap((h2) => [
    h2.header,
    ...h2.lines.map((l2) => {
      const prefix = l2.type === "added" ? "+" : l2.type === "removed" ? "-" : " ";
      return prefix + l2.content;
    })
  ]).join("\n");
}

// src/DiffTracker.ts
var MAX_DIFF_LINES = 2e3;
var CONTEXT_LINES = 3;
function lcsTable(a2, b2) {
  ...
  for (let i2 = 1; i2 <= m2; i2++) {
    for (let j2 = 1; j2 <= n2; j2++) {
      dp[i2][j2] = a2[i2 - 1] === b2[j2 - 1] ? dp[i2 - 1][j2 - 1] + 1 : Math.max(dp[i2 - 1][j2], dp[i2][j2 - 1]);
    }
  }
  return dp;
}
function backtrack(dp, a2, b2) {
  const result = [];
  let i2 = a2.length, j2 = b2.length;
  while (i2 > 0 || j2 > 0) {
    if (i2 > 0 && j2 > 0 && a2[i2 - 1] === b2[j2 - 1]) {
      result.push({ type: "context", content: a2[i2 - 1], lineNoOld: i2, lineNoNew: j2 });
      i2--;
      j2--;
    } else if (j2 > 0 && (i2 === 0 || dp[i2][j2 - 1] >= dp[i2 - 1][j2])) {
      result.push({ type: "added", content: b2[j2 - 1], lineNoOld: null, lineNoNew: j2 });
      j2--;
    } else {
      result.push({ type: "removed", content: a2[i2 - 1], lineNoOld: i2, lineNoNew: null });
      i2--;
    }
  }
...
function buildHunks(lines) {
  ...
  const include = new Array(n2).fill(false);
  for (let i2 = 0; i2 < n2; i2++) {
    if (lines[i2].type !== "context") {
      const lo = Math.max(0, i2 - CONTEXT_LINES);
      const hi = Math.min(n2 - 1, i2 + CONTEXT_LINES);
  ...
  for (let i2 = 0; i2 <= n2; i2++) {
    if (i2 < n2 && include[i2]) {
      ...
      ranges.push([start, i2 - 1]);
      start = -1;
    }
  }
  return ranges.map(([s2, e2]) => {
    const hunkLines = lines.slice(s2, e2 + 1);
    const addedCount = hunkLines.filter((l2) => l2.type === "added").length;
    const removedCount = hunkLines.filter((l2) => l2.type === "removed").length;
    const firstOld = hunkLines.find((l2) => l2.lineNoOld !== null)?.lineNoOld ?? 1;
    const firstNew = hunkLines.find((l2) => l2.lineNoNew !== null)?.lineNoNew ?? 1;
    const oldCount = hunkLines.filter((l2) => l2.lineNoOld !== null).length;
    const newCount = hunkLines.filter((l2) => l2.lineNoNew !== null).length;
...
function computeDiff(oldText, newText) {
  ...
  const addedLines = flat.filter((l2) => l2.type === "added").length;
  const removedLines = flat.filter((l2) => l2.type === "removed").length;
  return { hunks, addedLines, removedLines, skippedDiff: false };
}
var DiffTracker = class {
  constructor() {
    this._cache = /* @__PURE__ */ new Map();
    this._changes = [];
    this._onFileChange = new vscode.EventEmitter();
    this._onConfidenceNeeded = new vscode.EventEmitter();
    this.onFileChange = this._onFileChange.event;
    this.onConfidenceNeeded = this._onConfidenceNeeded.event;
    const autoStart = vscode.workspace.getConfiguration("gofi").get("autoStart", true);
    this._isRunning = autoStart;
    this._startTime = autoStart ? /* @__PURE__ */ new Date() : null;
  }
  get isRunning() {
  ...
  get stats() {
    return {
      ...
      totalAddedLines: this._changes.reduce((s2, c2) => s2 + c2.addedLines, 0),
      totalRemovedLines: this._changes.reduce((s2, c2) => s2 + c2.removedLines, 0),
      totalEstimatedTokens: this._changes.reduce((s2, c2) => s2 + c2.estimatedTokens, 0),
  ...
  async handleFileEvent(uri, changeType) {
    ...
    if (addedLines === 0 && removedLines === 0 && !skippedDiff) {
      return;
    }
    const diffText = rawDiffText(hunks);
    const estimatedTokens = estimateTokens(diffText);
    const costPerMillion = vscode.workspace.getConfiguration("gofi").get("costPerMillionTokens", 3);
    ...
    if (vscode.workspace.getConfiguration("gofi").get("enableConfidenceScoring")) {
      this._onConfidenceNeeded.fire(change);
    }
  }
  applyConfidence(changeId, confidence) {
    const change = this._changes.find((c2) => c2.id === changeId);
    if (change) {
      change.confidence = confidence;
    }
    return change;
  }
  enrichWithHookContext(event) {
    const filePath = event.tool_input.file_path ?? event.tool_input.path;
    if (!filePath) {
      return;
    }
    const now = Date.now();
    const recent = [...this._changes].reverse().find(
      (c2) => (c2.filePath === filePath || c2.filePath.endsWith("/" + filePath) || filePath.endsWith("/" + c2.relPath)) && now - new Date(c2.timestamp).getTime() < 3e3
    );
    if (recent) {
      recent.hookContext = {
...
var FSWatcher = class {
  constructor(_diffTracker) {
    this._diffTracker = _diffTracker;
    this._disposables = [];
    this._debounceTimers = /* @__PURE__ */ new Map();
    this._excludeGlobs = vscode2.workspace.getConfiguration("gofi").get("excludeGlobs", ["**/node_modules/**", "**/.git/**", "**/dist/**", "**/out/**"]);
  }
  start() {
    ...
    for (const folder of folders) {
      const pattern = new vscode2.RelativePattern(folder, "**/*");
      const watcher = vscode2.workspace.createFileSystemWatcher(pattern, false, false, false);
    ...
    vscode2.workspace.textDocuments.forEach((doc) => this._diffTracker.seedCache(doc));
    this._disposables.push(
      vscode2.workspace.onDidOpenTextDocument((doc) => this._diffTracker.seedCache(doc))
    );
  }
  _schedule(uri, changeType) {
    if (this._shouldExclude(uri.fsPath)) {
      return;
    }
  ...
  dispose() {
    for (const t2 of this._debounceTimers.values()) {
      clearTimeout(t2);
    }
    this._debounceTimers.clear();
    this._disposables.forEach((d2) => d2.dispose());
  }
};
...
var HookWatcher = class {
  constructor(logPath) {
    this._onEvent = new vscode3.EventEmitter();
    this.onEvent = this._onEvent.event;
    this._watcher = null;
    this._offset = 0;
    const configured = vscode3.workspace.getConfiguration("gofi").get("hookEventsPath", "");
    this._logPath = logPath ?? configured ?? path2.join(os.homedir(), ".claude", "gofi-events.jsonl");
  }
  get logPath() {
  ...
  _tryWatch() {
    try {
      ...
      const dir = path2.dirname(this._logPath);
      if (!fs.existsSync(dir)) {
        return;
      }
      this._watcher = fs.watch(dir, (eventType, filename) => {
        if (filename && path2.join(dir, filename) === this._logPath) {
          this._readNewLines();
        }
      });
  ...
  _readNewLines() {
    try {
      ...
      if (!stat || stat.size <= this._offset) {
        return;
      }
...
// src/PromptWatcher.ts
var vscode4 = __toESM(require("vscode"));
var fs2 = __toESM(require("fs"));
var os2 = __toESM(require("os"));
var path3 = __toESM(require("path"));
var PromptWatcher = class {
  constructor() {
    this._onPrompt = new vscode4.EventEmitter();
    this.onPrompt = this._onPrompt.event;
    this._offsets = /* @__PURE__ */ new Map();
    this._watcher = null;
  ...
  start() {
    ...
    this._sessionDir = path3.join(os2.homedir(), ".claude", "projects", projectHash);
    if (!fs2.existsSync(this._sessionDir)) {
      return;
    }
    this._seedOffsets();
    this._watcher = fs2.watch(this._sessionDir, (_evt, filename) => {
      if (filename?.endsWith(".jsonl")) {
        this._readNewLines(path3.join(this._sessionDir, filename));
      }
    });
    this._watcher.on("error", () => {
    });
  ...
  _seedOffsets() {
    ...
    try {
      fs2.readdirSync(this._sessionDir).filter((f2) => f2.endsWith(".jsonl")).forEach((f2) => {
        const fp = path3.join(this._sessionDir, f2);
        const stat = fs2.statSync(fp, { throwIfNoEntry: false });
        if (stat) {
          this._offsets.set(fp, stat.size);
        }
      });
    } catch {
    }
...
// src/ConfidenceService.ts
var ClaudeConfidenceProvider = class {
  constructor() {
    this.name = "claude";
    this._client = null;
    this._cache = /* @__PURE__ */ new Map();
...
var NoOpProvider = class {
  ...
  async scoreChange(_change) {
    return null;
  }
  async scorePrompt(_prompt) {
    return null;
  }
};
...
async function installClaudeHook(context) {
  ...
  try {
    ...
    const idx = existing.findIndex((e2) => e2.matcher === hookEntry.matcher);
    if (idx >= 0) {
